...
def check_state(state):
    ...
    if not (state & STATE_CODE_LOCK):
        raise Exception("STATE_CODE_LOCK not valid")

    if not (state & STATE_TOW_DECODED):
        raise Exception("STATE_TOW_DECODED not valid")
...
def rinex_header(runby=None, marker=None, observer=None, agency=None,
    ...
    h += "{0:<20}{1:<20}{2:<20}PGM / RUN BY / DATE\n".format(PGM, runby if runby is not None else "UNKNOWN", datestr)
    ...
    # Observer and agency
    h += "{0:<20}{0:<40}OBSERVER / AGENCY\n".format(observer if observer is not None else "UNKN",
                                                    agency if agency is not None else "UNKN")
    
    # Receiver line
    h += "{0:<20}{1:<20}{2:<20}REC # / TYPE / VERS\n".format(receiver if receiver is not None else "UNKN",
                                                    rxtype if rxtype is not None else "UNKN",
                                                    version)
    
    # Antenna type
    h += "{0:<20}{1:<40}ANT # / TYPE\n".format(antenna if antenna is not None else "UNKN",
                                               anttype if anttype is not None else "UNKN")
